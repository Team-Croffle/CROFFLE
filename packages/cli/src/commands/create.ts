  ...
  .action(async (projectNameArg) => {
    try {
      ...
      if (fs.existsSync(targetPath)) {
        console.error(picocolors.red(`Target directory ${projectName} already exists!`));
        process.exit(1);
      }
      ...
      if (!fs.existsSync(templateDir)) {
        console.error(picocolors.red(`Template ${answers.template} not found at ${templateDir}.`));
        process.exit(1);
      }
      ...
      console.log(
        picocolors.dim('Tip: package with  pnpm dlx @croffledev/croffle-cli pack  after build'),
      );
    } catch (error) {

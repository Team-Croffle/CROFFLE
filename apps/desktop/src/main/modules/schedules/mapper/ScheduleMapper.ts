import type { Schedule as ScheduleEntity } from '../model/Schedule';
...
export const ScheduleMapper = {
  ...
  toEntity(data: Partial<ScheduleInterface>): Partial<ScheduleEntity> {
    ...
    if (data.startDate) {
      entityData.startDate = new Date(data.startDate);
    }
    if (data.endDate) {
      entityData.endDate = new Date(data.endDate);
    }
    if (data.createdAt) {
      entityData.createdAt = new Date(data.createdAt);
    }
    if (data.updatedAt) {
      entityData.updatedAt = new Date(data.updatedAt);
    }
    return entityData;
  },
};
